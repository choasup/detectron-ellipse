...
def add_fpn_retinanet_outputs(model, blobs_in, dim_in, spatial_scales):
    ...
    bbox_regr_dim = (
        4 * (model.num_classes - 1) if cfg.RETINANET.CLASS_SPECIFIC_BBOX else 4 # (4+36) || 4
    )	
            ...
            for nconv in range(cfg.RETINANET.NUM_CONVS):
                ...
                if lvl == k_min:
                    ...
                    bl_out = model.ConvShared(
                        ...
                        stride=1,
                        pad=1,
                        weight='retnet_cos_conv_n{}_fpn{}_w'.format(
                            nconv, k_min
                        ),
                        bias='retnet_cos_conv_n{}_fpn{}_b'.format(
                            nconv, k_min
                        )
                    )
                bl_in = model.Relu(bl_out, bl_out)
                ...
                # At least 1 convolution for dealing different aspect ratios
                bl_feat = bl_in
            cos_feat_list.append(bl_feat)

    # Depending on the features [shared/separate] for bbox, add prediction layer
    for i, lvl in enumerate(range(k_min, k_max + 1)):
        bbox_pred = 'retnet_cos_pred_fpn{}'.format(lvl)
        bl_feat = cos_feat_list[i]
        if lvl == k_min:
            model.Conv(
                bl_feat,
                bbox_pred,
            ...
            model.ConvShared(
                ...
                pad=1,
                stride=1,
                weight='retnet_cos_pred_fpn{}_w'.format(k_min),
                bias='retnet_cos_pred_fpn{}_b'.format(k_min)
            )
...
def add_fpn_retinanet_losses(model):
    ...
    # ------------------------------- new add ---------------------------------
    # ==========================================================================
    # cos regression loss - SelectSmoothL1Loss for multiple anchors at a location
    # ==========================================================================
    
    for lvl in range(k_min, k_max + 1):
        suffix = 'fpn{}'.format(lvl)
        cos_loss = model.net.SelectSmoothL1Loss(
            [
                'retnet_cos_pred_' + suffix,
