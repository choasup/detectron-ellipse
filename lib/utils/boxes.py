...
# ------------------------ rect ------------------------
def rect_transform_inv(boxes, gt_rect, weights=(1.0, 1.0, 1.0, 1.0)):
...
def cos_transform(cos_deltas):
    num_cos = cos_deltas.shape[1]

    cos_v = np.cos(np.linspace(0, 360.0 - 360.0 / num_cos, num_cos) / 180.0 * math.pi)
    sin_v = np.sin(np.linspace(0, 360.0 - 360.0 / num_cos, num_cos) / 180.0 * math.pi)

    cos_sum = np.sum(cos_deltas * cos_v, 1)	
    sin_sum = np.sum(cos_deltas * sin_v, 1)	

    theta = np.arctan(sin_sum / cos_sum) / math.pi * 180.0	
    
    sub = np.intersect1d(np.where(sin_sum < 0), np.where(cos_sum < 0))
    add = np.intersect1d(np.where(sin_sum > 0), np.where(cos_sum < 0))
    
    theta[sub] = theta[sub] - 180.0
    theta[add] = theta[add] + 180.0
    
    return np.reshape(theta / 2.0, (1, -1))
