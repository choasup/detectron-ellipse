...
def im_detect_bbox(model, im, timers=None):
	...
	cos_preds.append(core.ScopedName('retnet_cos_pred_{}'.format(suffix)))
    ...
    # ----------------- cos_preds ----------------#
    cos_preds = workspace.FetchBlobs(cos_preds)		
	...
	# ---------- cos reshape ---------
	cos_pred = cos_pred.reshape((
	    cos_pred.shape[0], A, 4, cos_pred.shape[2], cos_pred.shape[3]))
	...
	# ------------------ press box & cos ----------------
	    ...
	    cos_deltas = cos_pred[0, anchor_ids, :, y, x]
        else:
	...
	#print("shape:", box_deltas.shape, cos_deltas.shape)

        pred_boxes = (
            box_utils.bbox_transform(boxes, box_deltas)
            if cfg.TEST.BBOX_REG else boxes)
 	
	cos_theta = box_utils.cos_transform(cos_deltas)
	#print(cos_theta)	
	...
	# ------------- edit box_scores --------------
   	box_scores = np.zeros((pred_boxes.shape[0], 6))
        box_scores[:, 0:4] = pred_boxes
        box_scores[:, 4] = scores
	#print(scores.shape)
	box_scores[:, 5] = cos_theta
    ...
    for cls, boxes in boxes_all.items():
        ...
        keep = box_utils.nms(cls_dets[:, 0:5], cfg.TEST.NMS)
        cls_dets = cls_dets[keep, :]
	
	# ------------------- edit out --------------
        out = np.zeros((len(keep), 7))
        out[:, 0:6] = cls_dets
        out[:, 6].fill(cls)
    ...
    for c in range(1, num_classes):
        inds = np.where(detections[:, 6] == c)[0] # 5 -> 6
        cls_boxes[c] = detections[inds, :6] # 5 -> 6
